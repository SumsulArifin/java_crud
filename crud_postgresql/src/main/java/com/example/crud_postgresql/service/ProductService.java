...
import com.example.crud_postgresql.dto.ProductMapper;
...
import com.example.crud_postgresql.mapper.ProductMapper;
...
public class ProductService {
    ...
    public ProductDTO createProduct(ProductDTO dto , Long categoryId ) {
        if (productRepository.findByNameIgnoreCase(dto.getName()).isPresent()) {
            throw new ValidationException("Product with name '" + dto.getName() + "' already exists.");
        }
        Category category = null;
        if (dto.getCategoryName() != null) {
            category = categoryRepository.findByName(dto.getCategoryName())
                    .orElseThrow(() -> new ResourceNotFoundException("Category not found with ID: " + dto.getCategoryName()));
        }
    ...
    public ProductDTO updateProduct(Long id, ProductDTO dto) {
        ...
        if (StringUtils.hasText(dto.getName()) && !dto.getName().equalsIgnoreCase(existing.getName())) {
            if (productRepository.findByNameIgnoreCase(dto.getName()).isPresent()) {
                throw new ValidationException("Product with name '" + dto.getName() + "' already exists.");
            }
        }
